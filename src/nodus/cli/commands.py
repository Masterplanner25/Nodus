...
_DETAILED_HELP: dict[str, str] = {
    ...
    "check": "\n".join([
        ...
        "",
        "A call to a name defined nowhere passes by default: a host-registered",
        "function is indistinguishable from a typo, and rejecting it would",
        "reject every embedded program. Declare the host surface to opt in --",
        "a file with any `extern` gets strict name resolution, so an undeclared",
        "unknown name is reported as the typo it is (#489):",
        "",
        "  extern delegate(who: string, task: string) -> string",
        "",
        "`nodus lsp` diagnostics are stricter regardless (undefined/unused",
        "variables, unreachable code, step bodies included).",
        "",
        "Options:",
        "  --project-root PATH    Override the project root directory",
        "",
    ...
    "serve": "\n".join([
        ...
        "",
        "Submitted code is confined: subprocess, network and environment access",
        "are denied unless granted below. `nodus run` is deliberately different --",
        "a script you wrote and chose to run is not code arriving over a socket.",
        "",
        "Options:",
        "  --host HOST                      Bind address (default: 127.0.0.1)",
        "  --port PORT                      Port to listen on (default: 7331)",
        "  --auth-token TOKEN               Require this token on all requests (required for non-local hosts)",
        "  --allow-paths PATHS              Colon-separated list of paths the runtime may access",
        "  --writable-paths PATHS           Subset of those that may be written (default: all)",
        "  --allow-input                    Allow scripts to read from stdin",
        "  --allow-subprocess               Allow submitted code to run subprocesses (denied by default)",
        "  --allow-network                  Allow submitted code to make network calls (denied by default)",
        "  --allow-env                      Allow submitted code to read the process environment (denied by default)",
        "  --allowed-commands NAMES         Comma-separated executables permitted when --allow-subprocess is set",
        "  --allowed-hosts HOSTS            Comma-separated hosts permitted when --allow-network is set",
        ...
        "NODUS_SERVER_ALLOW_NETWORK, NODUS_SERVER_ALLOW_ENV, NODUS_SERVER_ALLOWED_COMMANDS,",
        "NODUS_SERVER_ALLOWED_HOSTS.",
        "",
        "Examples:",
        "  nodus serve",
        "  nodus serve --host 0.0.0.0 --port 8080 --auth-token mysecret",
        "  nodus serve --allow-network --allowed-hosts api.example.com",
    ]),
    "worker": "\n".join([
