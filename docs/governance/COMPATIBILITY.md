...
- **v4.0.6 (2026-06-20, current):** Patch release. `@retry` annotation no-op fixed (COMPILER-001, #267); spurious "spawned task never executed" warning fixed (WARN-001, #268); `nodus serve --help` and `nodus worker --help` now print usage instead of starting. No bytecode break.
