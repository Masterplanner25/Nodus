...
- **#856: a coroutine spawned by a module function reached through a foreign closure ran on the wrong VM and was silently dropped.**
  ...
  A foreign context is now adopted onto this VM's builtins at the point it is installed (`VM._adopt_foreign_ctx`); the program half stays the caller's. `tests/test_foreign_ctx_keeps_own_builtins.py` pins it with a three-file repro, both entry points.

  Filed (open) while getting here, not fixed by this entry: #855 (the first HTTP call's client-construction cost), #857 (`nodus serve`'s unraisable 200 ms budget) and #858 (`POST /workflow/run` running a self-running program's workflow twice).
