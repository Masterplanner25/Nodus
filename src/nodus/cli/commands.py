...
COMMANDS: dict[str, Command] = {
    ...
    "graph": _c(
        ...
        subcommands={
            "run": (_PROJECT, frozenset()),
            "show": (_PROJECT | frozenset({"--format", "--output"}), frozenset()),
        },
    ),
    # -- Server ------------------------------------------------------------
    "serve": _c(
...
_DETAILED_HELP: dict[str, str] = {
    ...
    "graph": "\n".join([
        ...
        "  show <file> [--format FMT] [--output FILE] [--project-root PATH]",
        ...
        "",
        "An edge means 'B depends on A'. A step's `on: [...]` filter is not",
        "drawn, because the plan does not record it.",
        "",
        "Examples:",
